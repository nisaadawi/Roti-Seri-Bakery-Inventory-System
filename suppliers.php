...
<body>
    ...
    <div class="container">
        <div class="card">
            <div class="card-header">
                <a class="btn btn-sm btn-primary btn-flat" href="javascript:void(0)" onclick="showAddModal()">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-success btn-sm btn-flat dropdown-toggle" data-bs-toggle="dropdown">
                                    Action
                                </button>
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="javascript:void(0)" onclick="fetchSupplierData(<?= $row['num'] ?>)">Edit</a></li>
                                    <li><a class="dropdown-item" href="javascript:void(0)" onclick="confirmDelete(<?= $row['num'] ?>)">Delete</a></li>
                                </ul>
                            </td>
                        </tr>
                        <?php endwhile; ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="modal-title" class="modal-title">Add New Supplier</h5>
    ...
    <script>
        function fetchSupplierData(id) {
            fetch(`supplier_actions.php?action=fetch&id=${id}`)
                .then(response => response.json())
                .then(data => {
                    document.getElementById("modal-title").innerText = "Edit Supplier";
                    document.getElementById("supplier-id").value = id;
                    document.getElementById("supplier-code").value = data.code;
                    document.getElementById("supplier-name").value = data.supplier_name;
                    document.getElementById("supplier-contact").value = data.contact_number;
                    document.getElementById("supplier-ingredient").value = data.supplier_ingredient;
                    document.getElementById("supplier-price").value = data.current_price;
                    document.getElementById("supplier-measurement").value = data.measurement;
                    document.getElementById("supplier-performance").value = data.supplier_performance;
                    document.getElementById("modal-form").action = "supplier_actions.php?action=edit";
                    document.getElementById("modal").style.display = "block";
                })
                .catch(error => alert("Error fetching supplier data: " + error));
        }

        function confirmDelete(id) {
            if (confirm("Are you sure you want to delete this supplier?")) {
                window.location.href = `supplier_actions.php?action=delete&id=${id}`;
            }
        }
        ...
        function showAddModal() {
            ...
            document.getElementById("modal-form").action = "supplier_actions.php?action=add";
            document.getElementById("modal").style.display = "block";
        }
